...
def update_data(conn, table, set_columns, set_values, condition_column, condition_value):
...
def update_or_insert_vectordata(conn, schema_name, table_name,
                                id, title, description, content,
                                url, row_updated, embedded,
    ...
    try:
        ...
        if len(results) == 0:
            print('inserting')
            insert_data(conn, f'{schema_name}.{table_name}', ['id', 'title', 'description', 'content', 'url', 'row_updated', 'embedding'], [
                                id, title, description, content, url, row_updated, embedded])
        else:
            print('Updating')
            update_data(conn, f'{schema_name}.{table_name}', ['id', 'title', 'description', 'content','url','row_updated','embedding'], [id,title,description,content,url,row_updated,embedded], 'id', id)
